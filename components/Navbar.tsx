...
const NavItem:FunctionComponent<{
  ...
  return (
      activeItem !== name ? (
        <Link href={route}>
          <a>
            <span onClick={() => setActiveItem(name)} className="hover:text-red-500">{name}</span> 
          </a>
        </Link>
      ) : null 
  )
...
const Navbar = () => {
  ...
  return (
    <div className="flex justify-between px-5 py-3 my-3">
      <span className="text-xl font-bold text-yellow-500 border-b-4 border-red-500 md:text-2xl">{activeItem}</span>
